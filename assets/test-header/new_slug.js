...
$(function() {
    ...
    $(document).on('click', function(event) {
        if (!$(event.target).closest(userAccountPopover).length && !$(event.target).closest(showUserAccountPopover).length) {
            userAccountPopover.removeClass('visible');
        }
    });

    slideToggles.on( "click", function() {
        $(this).siblings('.hidden-info').slideToggle( "600", function() {
            // Animation complete.
        });
    ...
    console.log(carouselMenuItems.first().width());
    ...
    rightCarouselButton.on( "click", function() {
        ...
        if (carouselMenuWidth - widthToTranslate > areaWidth){
            carouselMenu.css({'transform': 'translateX(-' + widthToTranslate + 'px)',
                        'transition': 'transform 200ms'});
        }
        else {
            carouselMenu.css({'transform': 'translateX(-' + (carouselMenuWidth - areaWidth) + 'px)',
                        'transition': 'transform 200ms'});
    ...
    window.onscroll = function() {checkOffsetSicky()};
    ...
    $('#autocomplete-ajax').autocomplete({
        ...
        transformResult: function(response) {
            ...
            return {
                suggestions: $.map(object, function(dataItem) {
                    return { value: dataItem.title, data: { type: dataItem.type, url: dataItem.url} };
                })
            };
        },
        formatResult: function (suggestion, currentValue) {
            ...
            var suggestionHref = "<a href=" + suggestion.data.url + ">" + suggestion.value + "</a>";
